...
export default function ChangeHolidayTable({ data, loading, onDelete }: Props) {
  ...
  const columns: ColumnsType<ChangeHolidayResponse> = [
    {
      title: CHANGE_HOLIDAY_LABEL.TARGET_TYPE,
      dataIndex: "targetType",
      key: "targetType",
      render: (value: ChangeHolidayResponse["targetType"]) => {
        if (value === "employee") {
          return <Tag color="blue">{CHANGE_HOLIDAY_LABEL.TARGET_EMPLOYEE}</Tag>;
        }
        if (value === "payroll-group") {
          return (
            <Tag color="geekblue">
        ...
        return (
          <Tag color="purple">{CHANGE_HOLIDAY_LABEL.TARGET_EMPLOYEE_GROUP}</Tag>
        );
      },
    },
    {
      title: CHANGE_HOLIDAY_LABEL.TARGET,
