...
export default function ChangeHolidayList() {
  ...
  const [pending, setPending] = useState<ChangeHolidayFilter>(DEFAULT_FILTER);
  const [committed, setCommitted] =
    useState<ChangeHolidayFilter>(DEFAULT_FILTER);
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useChangeHolidays(committed, searchKey);
  ...
  const batchGroups = useMemo<BatchGroup[]>(() => {
    const map = new Map<string, ChangeHolidayResponse[]>();
    ...
    return Array.from(map.entries())
      .map(([batchCode, entries]) => ({
        ...
        holidayName: entries[0].holidayName,
  ...
  const entryColumns: TableColumnsType<ChangeHolidayResponse> = [
    {
    ...
    },
    {
      ...
      key: "holidayName",
      width: entryWidths.holidayName,
      onHeaderCell: () =>
        ({
          width: entryWidths.holidayName,
          onResize: (w: number) => entryResize("holidayName", w),
        }) as object,
    },
    {
  ...
  const batchEntryColumns: TableColumnsType<ChangeHolidayResponse> = [
    {
      title: CHANGE_HOLIDAY_LABEL.EMPLOYEE,
      dataIndex: "fullName",
      key: "fullName",
  ...
  const batchColumns: TableColumnsType<BatchGroup> = [
    ...
    },
    {
      ...
      dataIndex: "holidayName",
      key: "holidayName",
      width: batchWidths.holidayName,
      onHeaderCell: () =>
        ({
          width: batchWidths.holidayName,
          onResize: (w: number) => batchResize("holidayName", w),
        }) as object,
    },
    {
      title: "Date Range",
    ...
    <div className="content-page">
      ...
      <Form layout="vertical" className="mb-4">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-x-4 items-end">
          <Form.Item
            label={CHANGE_HOLIDAY_LABEL.FILTER_FROM_DATE}
            className="mb-0"
          >
            <DatePicker
              style={{ width: "100%" }}
          ...
          <Form.Item
            label={CHANGE_HOLIDAY_LABEL.FILTER_TO_DATE}
            className="mb-0"
          >
            <DatePicker
              style={{ width: "100%" }}
